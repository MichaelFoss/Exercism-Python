...
def categorize_dish(dish_name: str, dish_ingredients: list[str]) -> str:
    ...
    for (label, category_ingredients) in categories:
        if dish_ingredients <= category_ingredients:
            return f'{dish_name}: {label}'
    raise RuntimeError(f'dish_name "{dish_name}" not in category ingredients (this should never happen)')
...
def singleton_ingredients(dishes: list[set[str]], intersection: set[str]) -> set[str]:
    ...
    for dish in dishes:
        unique_ingredients = unique_ingredients ^ (dish - intersection)
